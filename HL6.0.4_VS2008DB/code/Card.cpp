...
BOOL CCard::LoadCardLib()
{
	...
	m_pSetMotorDelay		= (T_HS_SetMotorDelay)GetProcAddress(m_hMarkLib,LPCSTR(75));
...
BOOL CCard::GetMarkRect(double& x,double& y)
{
#ifdef _SOFTMODE_EDITOR_
	x = 160;
	y = 160;
#else
	if ( m_pGetMarkRange != NULL && m_bInitOK )
	{
		return (0 == m_pGetMarkRange(&x,&y));
	}
	else
	{
		x = 160;
...
BOOL CCard::Init_SetLayerPara()
{
	...
	{	
		...
		case eType_YLPV2_CLASSIC:
			m_strEMCCLaser = _T("YLPV2_CLASSIC");
			break;
		case eType_YLPV2_GENERAL:
			m_strEMCCLaser = _T("YLPV2_GENERAL");
			break;
		case eType_YLPP_CLASSIC:
			m_strEMCCLaser = _T("YLPP_CLASSIC");
			break;

		case eType_YLPP_GENERAL:
			m_strEMCCLaser = _T("YLPP_GENERAL");
			break;

		case eType_HFMA_CLASSIC:
			m_strEMCCLaser = _T("HFMA_CLASSIC");
			break;

		case eType_HFMA_GENERAL:
			m_strEMCCLaser = _T("HFMA_GENERAL");
			break;
	}
...
UINT CCard::SetMotorDelay(int nDelay)
{
	if( !m_pSetMotorDelay )
		return HLERROR;

	return m_pSetMotorDelay(nDelay);
}
