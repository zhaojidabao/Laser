...
{
	if ( m_stkCtrlDot.m_dotArray.GetSize()  !=  2 ) // 不构成圆无控制点
		return;

	CDot dot1 = m_stkCtrlDot.m_dotArray.GetAt(0);	// 圆心
	...
	assistDot.y = dot1.y;
	assistDot.z = 2;
	pointArray.Add(assistDot);	// 圆心

	assistDot.x = dot1.x;
	assistDot.y = dot1.y + dbRadius;
	assistDot.z = 3;
	pointArray.Add(assistDot);	// 平行x切线上面切点

	assistDot.x = dot1.x;
	assistDot.y = dot1.y - dbRadius;
	assistDot.z = 3;
	pointArray.Add(assistDot);	// 平行x切线下面切点

	assistDot.x = dot1.x - dbRadius ;
	assistDot.y = dot1.y;
	assistDot.z = 3;
	pointArray.Add(assistDot);	// y左切点

	assistDot.x = dot1.x + dbRadius;
	assistDot.y = dot1.y;
	assistDot.z = 3;
	pointArray.Add(assistDot);	// y右切点
}
